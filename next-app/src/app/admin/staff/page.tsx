    ...
    <div className="max-w-7xl mx-auto pb-12">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-10">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Verwalten Sie Ihre Mitarbeiter, Profile und Qualifikationen.</p>
        </div>
                ...
                <Link key={member.id} href={`/admin/staff/${member.id}`}>
                    <IOSCard className="p-0 overflow-hidden group hover:shadow-2xl hover:scale-[1.02] transition-all duration-500 border-black/5 h-full flex flex-col">
                        <div className="p-8 flex-1">
                            ...
                            <h3 className="text-xl font-black text-gray-900 mb-1 group-hover:text-[#667eea] transition-colors">{member.full_name}</h3>
                            <p className="text-xs font-bold text-gray-400 truncate mb-6">{member.email}</p>
                            
                            <div className="flex items-center gap-2 text-[10px] font-black text-black/20 uppercase tracking-widest italic">
                                <span>📱</span>
                                <span className="group-hover:text-gray-800 transition-colors">{member.phone || 'Keine Nummer'}</span>
                            </div>
                        </div>
