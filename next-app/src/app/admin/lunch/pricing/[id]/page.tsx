...
const ROUTE = 'admin.lunch.pricing.id'
...
export default function AdminLunchPricingEditPage() {
  const { t } = useI18n()
  ...
  const [form, setForm] = useState<LunchPricingFormState>({
    group_id: '',
    price_per_meal: 0,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await supabase.from('lunch_pricing').update(payload).eq('id', pricingId).select().single()
  ...
  const handleDelete = async () => {
    ...
    try {
      await supabase.from('lunch_pricing').delete().eq('id', pricingId)
        ...
        {loading ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex gap-3 justify-end pt-4 border-t">
              <IOSButton type="button" variant="secondary" onClick={handleDelete} disabled={submitting}>
                Delete
              </IOSButton>
              <IOSButton type="submit" disabled={submitting}>
                {submitting ? (
