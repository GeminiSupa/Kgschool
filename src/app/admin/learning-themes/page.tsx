    ...
    <div className="max-w-7xl mx-auto pb-20">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
        ...
        <div className="flex gap-3">
          <Link href="/admin/learning-themes/new">
            <IOSButton>{t(sT('createNewTheme'))}</IOSButton>
          </Link>
        </div>
      </div>
      ...
      <div className="mb-10">
        <div className="relative inline-block w-full md:w-64">
            <select 
                ...
                className="w-full min-h-11 pl-6 pr-12 py-3 bg-background border border-border rounded-2xl text-[11px] font-black text-foreground uppercase tracking-widest shadow-sm focus:ring-2 focus:ring-(--aura-primary)/25 outline-none transition-all appearance-none cursor-pointer"
            >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
            {learningThemes.map(theme => (
                <IOSCard key={theme.id} className="p-0 overflow-hidden group hover:border-aura-primary/20 transition-all duration-500">
                    <div className="h-56 bg-slate-100 overflow-hidden relative">
                        {theme.photos?.[0] ? (
                             ...
                             <div className="w-full h-full flex flex-col items-center justify-center text-6xl bg-aura-primary/10 text-aura-primary/20 group-hover:bg-aura-primary/15 group-hover:text-aura-primary/30 transition-colors">
                                📚
                                <span className="text-[10px] font-black uppercase tracking-widest mt-4 opacity-40">Kein Bild</span>
                             </div>
                        )}
                    ...
                    <div className="p-10">
                        <div className="flex items-center gap-2 mb-2">
                            ...
                            <p className="text-[10px] font-black text-aura-primary uppercase tracking-widest leading-none">{getGroupName(theme.group_id || '')}</p>
                        </div>
                        <h3 className="text-2xl font-black text-slate-900 dark:text-slate-50 mb-4 group-hover:text-aura-primary transition-colors tracking-tight">{theme.title}</h3>
                        ...
                        <div className="flex gap-4 pt-8 border-t border-slate-50">
                             ...
                             <button className="w-12 h-12 bg-aura-primary/10 text-aura-primary rounded-xl flex items-center justify-center hover:bg-aura-primary hover:text-white transition-all shadow-sm">
                                →
                             </button>
                        </div>
                    </div>
