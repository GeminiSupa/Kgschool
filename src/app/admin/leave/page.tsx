    ...
    <div className="max-w-7xl mx-auto pb-12">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-10">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Verwalten Sie Urlaubsanträge und Krankmeldungen von Kindern und Personal.</p>
        </div>
      </div>
      ...
      <div className="mb-10 flex gap-2">
            <button 
                ...
                className={`px-8 py-2.5 rounded-2xl text-[10px] font-black uppercase tracking-widest border transition-all duration-300 ${
                    activeTab === 'children' ? 'bg-black text-white border-black shadow-lg' : 'bg-white text-gray-400 border-black/5 hover:border-black/10'
                }`}
            >
                Kinder
            </button>
            <button 
                ...
                className={`px-8 py-2.5 rounded-2xl text-[10px] font-black uppercase tracking-widest border transition-all duration-300 ${
                    activeTab === 'staff' ? 'bg-black text-white border-black shadow-lg' : 'bg-white text-gray-400 border-black/5 hover:border-black/10'
                }`}
            >
                Personal
            </button>
      ...
      <div className="grid grid-cols-1 gap-4">
            {activeTab === 'children' ? (
                leaveRequests.length === 0 ? <p className="text-center py-20 text-gray-400 font-bold uppercase tracking-widest opacity-20 text-4xl">Leer</p> :
                leaveRequests.map(req => (
                    <IOSCard key={req.id} className="p-0 overflow-hidden border-black/5 group">
                        <div className="p-6 md:p-8 flex flex-col md:flex-row md:items-center justify-between gap-6">
                            <div className="flex-1">
                                <div className="flex items-center gap-3 mb-2">
                                    <h3 className="text-lg font-black text-gray-900">{getChildName(req.child_id)}</h3>
                ...
                teacherRequests.length === 0 ? <p className="text-center py-20 text-gray-400 font-bold uppercase tracking-widest opacity-20 text-4xl">Leer</p> :
                teacherRequests.map(req => (
                    <IOSCard key={req.id} className="p-0 overflow-hidden border-black/5 group">
                        <div className="p-6 md:p-8 flex flex-col md:flex-row md:items-center justify-between gap-6">
                            <div className="flex-1">
                                <div className="flex items-center gap-3 mb-2">
                                    <h3 className="text-lg font-black text-gray-900">{getStaffName(req.teacher_id)}</h3>
                                    <span className={`px-2 py-0.5 text-[9px] font-black uppercase rounded-md border ${
                                        req.status === 'approved' ? 'bg-green-50 text-green-600 border-green-100' : 
                                        req.status === 'rejected' ? 'bg-red-50 text-red-600 border-red-100' :
                                        'bg-amber-50 text-amber-600 border-amber-100'
