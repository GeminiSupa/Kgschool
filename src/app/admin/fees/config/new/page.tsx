...
export default function FeeConfigFormPage() {
  const router = useRouter()
  const { id } = useParams()
  const isEdit = !!id && id !== 'new'
  
  const { configs, fetchConfigs, createConfig, updateConfig } = useFeeConfigStore()
  ...
  }, [fetchGroups, fetchConfigs, isEdit])

  useEffect(() => {
    if (isEdit && configs.length > 0) {
