  ...
  return (
    <div className="max-w-7xl mx-auto pb-12">
      <div className="mb-10 text-center md:text-left">
        ...
        <p className="text-sm text-gray-500 mt-1">Eigene Abwesenheiten und Kita-Veranstaltungen.</p>
      </div>
        ...
        <div className="lg:col-span-3">
            <IOSCard className="p-8 border-black/5 shadow-sm">
                <div className="flex items-center justify-between mb-8">
                    <h3 className="text-xl font-black text-gray-900">{new Date(currentYear, currentMonth).toLocaleString('de-DE', { month: 'long', year: 'numeric' })}</h3>
                    <div className="flex gap-2">
                        <button className="p-2 hover:bg-gray-100 rounded-lg transition-colors text-xl">‹</button>
                        <button className="p-2 hover:bg-gray-100 rounded-lg transition-colors text-xl">›</button>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-7 gap-2">
                    ...
                    {calendarDays.map(day => (
                        <div key={day} className="aspect-square bg-gray-50/50 rounded-2xl border border-black/5 p-2 group hover:border-[#667eea]/20 transition-all cursor-pointer relative overflow-hidden">
                            <span className="text-xs font-black text-gray-400 group-hover:text-black transition-colors">{day}</span>
                        </div>
                    ))}
                </div>
            </IOSCard>
        ...
        <div className="space-y-6">
            <IOSCard className="p-6 border-black/5 bg-[#667eea]/5">
                ...
                <div className="space-y-4">
                    {requests.length === 0 ? (
                         <p className="text-[10px] font-black text-gray-400 italic">Keine geplanten Urlaube diesen Monat.</p>
                    ) : (
                        requests.map(req => (
                            <div key={req.id} className="p-4 bg-white rounded-2xl border border-black/5 shadow-sm">
                                <p className="text-xs font-black text-gray-900">Urlaub genehmigt</p>
