  ...
  return (
    <div className="max-w-4xl mx-auto pb-12">
      <div className="mb-10 text-center">
        ...
        <p className="text-sm text-ui-soft mt-1">Dokumentation der pädagogischen Entwicklung Ihres Kindes.</p>
      </div>
             ...
             {children.map(c => (
                <button
                    ...
                    className={`px-6 py-2 rounded-2xl text-[10px] font-black uppercase tracking-widest border transition-all duration-300 ${
                        selectedChildId === c.id ? 'bg-black text-white' : 'bg-white text-ui-soft border-black/5'
                    }`}
                >
                    {c.first_name}
                </button>
      ...
      {observations.length === 0 ? (
        <IOSCard className="p-20 text-center bg-gray-50/30 border-black/5">
          ...
          <p className="text-ui-soft font-bold max-w-xs mx-auto">Noch keine Beobachtungen eingetragen.</p>
        </IOSCard>
      ) : (
            ...
            {observations.map(obs => (
                <IOSCard key={obs.id} className="p-8 border-black/5 hover:border-indigo-500/30 transition-all">
                    <div className="flex justify-between items-start mb-6">
                        <div>
                             <h3 className="text-lg font-black text-slate-900 dark:text-slate-50">{obs.title || 'Entwicklungsbeobachtung'}</h3>
                        ...
                        <span className="text-xs font-black text-ui-soft uppercase tracking-widest bg-gray-100 px-3 py-1 rounded-full">{obs.observation_type}</span>
                    </div>
                    <p className="text-base text-ui-muted font-medium leading-relaxed mb-6">{obs.content}</p>
